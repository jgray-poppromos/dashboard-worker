...
* **2026-04-07**: Updated JSON parsing of KV items in `dashboard-worker.js` with `.catch(() => null)` to ignore invalid data causing 500 errors. Fixed UI `dedupeEvents` logic to fall back gracefully when encountering old events missing a timestamp, resolving the "Disconnected" error feed loop. Disabled the flawed `autoClearFromReports` logic in `index.html` to prevent active webhooks from being dismissed accidentally due to Sugar CRM report lag.
